...
Budgetzilla can optionally sync your data to Google Drive for backup and cross-device access.

## How It Works

- Your data is exported as a file to your Google Drive
- The file is stored in a Budgetzilla app folder
...
Budgetzilla requests minimal permissions:

- **App folder access only** — Budgetzilla can only access its own folder
...
1. Set up Budgetzilla on each device
2. Connect the same Google account
3. Sync before and after making changes

:::caution
Budgetzilla is designed as a single-user app. Simultaneous edits from multiple devices may cause conflicts.
:::
...
- Budgetzilla servers never see your financial data
